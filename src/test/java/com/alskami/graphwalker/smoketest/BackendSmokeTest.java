...
import com.alskami.simulators.ClientSimulatorFactory;
...
public class BackendSmokeTest extends GraphwalkerTestBase implements Model_smoke_backend {
    ...
    public void e_add10dollars() {
        setPreviousClientSimulator(ClientSimulator.copy(getCurrentClientSimulator()));
    ...
    public void e_login() {
        ...
        String username = "";
        if(null == getCurrentClientSimulator()){
            setCurrentClientSimulator(ClientSimulatorFactory.getMainWebSimulator());
            getCurrentClientSimulator().getLoginController().login(username,getDefaultPassword());
        }else {
            getCurrentClientSimulator().getLoginController().login(username, getDefaultPassword());
        }
    ...
    public void e_log_out() {
        ...
        getCurrentClientSimulator().getLoginController().logout();
    }
    ...
    public void e_signup() {
        ...
        setCurrentClientSimulator(ClientSimulatorFactory.getMainWebSimulator());
        getCurrentClientSimulator().getSignUpController().signupUser("TheKing", "InTheGhetto", "Elvis", "Presley");
