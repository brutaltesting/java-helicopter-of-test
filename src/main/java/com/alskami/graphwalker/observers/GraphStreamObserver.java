            ...
            if (EventType.BEFORE_ELEMENT.equals(eventType)) {
                ...
                if (element instanceof RuntimeVertex) {
                    ...
                    if (vertex.hasSharedState() &&
                        ...
                        {
                            graph.addEdge(
                                    getId(lastContext, lastElement) + getId(machine.getCurrentContext(), vertex),
                                    (Node) graph.getNode(getId(lastContext, lastElement)),
                                    (Node) graph.getNode(getId(machine.getCurrentContext(), vertex)),
                                    true);
                        }
                    }
                } else {
                    ...
                    if (null == graph.getEdge(getId(machine.getCurrentContext(), edge))) {
                        ...
                        Edge arc = graph.addEdge(
                                getId(machine.getCurrentContext(), edge),
                                source,
                                getId(machine.getCurrentContext(),edge.getTargetVertex()),
                                true);
